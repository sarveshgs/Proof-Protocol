...
ContractInteractHelper.prototype = {
  ...
  performSignedTransaction: async function (params, returnType) {
        const oThis = this
        ;
    ...
    let nonce = await params.web3Provider.eth.getTransactionCount(params.senderAddress);

    console.log("eth.getTransactionCount(params.senderAddress).toNumber()", nonce);
        oThis.rawTx = {
    ...
    console.log(oThis.rawTx);

        const tx = new Tx(oThis.rawTx);

    var privateKey = coreConstant.PROOF_VERIFIER_PRIVATE_KEY;
