...
ProofOfStake.prototype = {
    ...
    startVoting: async function (senderAddress, senderPassphrase, gasPrice, options) {
        ...
        try {
          ...
          let transactionPromise = helper.performSignedTransaction(params, returnType);
          console.log("transaction Promise ", transactionPromise);
          return Promise.resolve(transactionPromise);
        } catch(err) {
            let errorParams = {
    ...
    registerToVote: async function (senderAddress, senderPassphrase, gasPrice, options) {
        ...
        try {
            ...
            return Promise.resolve(helper.performSignedTransaction(params, returnType));
        } catch(err) {
            let errorParams = {
